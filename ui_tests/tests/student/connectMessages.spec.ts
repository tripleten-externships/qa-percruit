...
// Before hook: Launch a new browser and page before each scenario and initialize page objects
  ...
  test('the user navigates to the Industry News page and industry news page displays', async ({ page }) => {
   ...
   // ------------------------ New Message Steps ------------------------
     ...
     await expect(page.locator(connectMessagesPage.MentorSearchInput)).toBeVisible();
   ...
   // ------------------------ Mentor Info Not Available Steps ------------------------
     await connectMessagesPage.searchMentor('Nonexistent Mentor');
      ...
      await expect(page.locator(connectMessagesPage.MentorSearchInput)).toHaveCount(0); // or hidden
     await expect(page.locator(connectMessagesPage.MessagesTitle)).toBeVisible();
   // ------------------------ Filter Unread ------------------------
