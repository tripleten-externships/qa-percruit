...
declare const page: Page;
    ...
    And('the Admin is on the Notifications tab in Profile Settings', async function () {
    ...
    Then('the Email Notifications section is visible', async function () {
      await expect(getByText('Email NotificationsMessage')).toBeVisible();
    });

    And('the preferences Message Notifications, Task & Goal Notifications, and Resume Review Notifications are available', async function () {
      await expect(getByRole('heading', { name: 'Message Notifications'})).toBeVisible({ timeout });
      await expect(getByRole('heading', { name: 'Task & Goal Notifications'})).toBeVisible({ timeout });
      await expect(getByRole('heading', { name: 'Resume Review Notifications'})).toBeVisible({ timeout });
    });

    And('each preference displays descriptive guidance about when emails are sent', async function () {
    ...
    And('the current state of each preference is clearly indicated', async function () {
    await page.getByRole('switch', { name: 'Message Notifications Receive' }).isVisible({ timeout });
    await page.getByRole('switch', { name: 'Task & Goal Notifications' }).isVisible({ timeout });
    await page.getByRole('switch', { name: 'Resume Review Notifications' }).isVisible({ timeout });
    });
...
function getByRole(arg0: string, arg1: { name: string; }): any {
  throw new Error('Function not implemented.');
}