...
export class LogoutPage extends BasePage {
  ...
  async isOnLoginPage() {
    await this.page
      .getByRole('heading', { name: 'Welcome to Percruit' })
      .click();
    return true;
  }
  //confirms student dashboard text is visible
  async isOnHomePage() {
    await expect(
      this.page.getByRole('heading', { name: 'Student Dashboard' })
    ).toBeVisible({ timeout: 30000 });
  }
  //checks student dashboard text is no longer visible to confirm a successful logout
  async noLongerOnDashboard() {
    const locator = this.page.locator('Student Dashboard');
