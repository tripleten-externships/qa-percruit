...
// ProfilePage implementation may not exist in all environments; use a loose type to avoid compile errors.
// At runtime the actual class is constructed from this.page so `any` preserves behavior.
type ProfilePageType = any;
const ProfilePageCtor: any = (global as any).ProfilePage ?? BasePage;
...
async function detectTimezoneExists(profile: ProfilePageType): Promise<boolean> {
...
Then('the admins display name and email are shown', async function (this: CustomWorld) {
    ...
    const expectedAdminName = (env as any).getAdminDisplayName?.() ?? process.env.ADMIN_DISPLAY_NAME;
    const expectedAdminEmail = (env as any).getAdminEmail?.() || process.env.ADMIN_EMAIL || 'admin@percruit.com';
...
Then('The displayed name, email, and timezone match the accounts stored values', async function (this: CustomWorld) {
    ...
    const expectedName = (env as any).getAdminDisplayName?.() ?? process.env.ADMIN_DISPLAY_NAME;
    const expectedEmail = (env as any).getAdminEmail?.() || process.env.ADMIN_EMAIL || 'admin@percruit.com';
